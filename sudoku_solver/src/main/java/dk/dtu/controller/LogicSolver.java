...
public class LogicSolver {
    ...
    // Takes a normal board, solves and verifies it. Returns true if it can be
    // solved deterministically, i.e. no guesses are required to solve the sudoku
    ...
    // Generates a unit (BigInteger[][]) for each rule of the sudoku, 0 = rows, 1 =
    // columns, 2 = boxes.
    private static BigInteger[][][] boardUnits(BigInteger[][] board) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                unitsRow[i][j] = x;
                unitsCol[j][i] = x;
                unitsBox[(j / boxSize + (i / boxSize) * boxSize)][((i - (i / boxSize * boxSize)) * boxSize
                        + (j - (j / boxSize * boxSize)))] = x;
            }
        }
        return new BigInteger[][][] { unitsRow, unitsCol, unitsBox };
    }

    // Takes an integer values, and represents that value as the index in a binary
    // string when "oneOrZero" = false (0). i.e. 1=0001, 2=0010, 3=0100, ...
    // When "oneOrZero" = true (1), a string of 1's the size of the inputvalue is
    // created, this is used for the empty fields on the board, with the input being
    ...
    // Takes the original board, and creates a copy of it where each cell is defined
    // by a binary value.
    ...
    // Sudoku strategy, takes an empty spot and eliminates all possible numbers
    // which are already found in the same rows, columns and boxes.
    private static void nakedSingles(BigInteger[][][] units, BigInteger[][] board, List<int[]> priority) {
        for (int h = 0; h < priority.size(); h++) {
            ...
            if (x.bitCount() == 1) {
                ...
                units[2][u][((row - (row / boxSize * boxSize)) * boxSize + (col - (col / boxSize * boxSize)))] = x
                        .not();
                priority.remove(h);
            }
        }
    ...
    // Sudoku strategy. Looks at the rows, columns and boxes individually and then
    // determines if there is a number that can only be placed in one spot.
        ...
        while (index < 3 && (unitRow < size || num <= size || unitCol < size)) {
            ...
            if (unitCol == size) {
                if (index == 0 && counter == 1) {
                    ...
                    units[2][(col / boxSize + (row / boxSize) * boxSize)][((row - (row / boxSize * boxSize)) * boxSize
                            + (col - (col / boxSize * boxSize)))] = testVal.not();
                    ...
                    units[2][(row / boxSize + (col / boxSize) * boxSize)][((col - (col / boxSize * boxSize)) * boxSize
                            + (row - (row / boxSize * boxSize)))] = testVal.not();
                    board[col][row] = testVal;

                    priority.remove(findIndex(priority, new int[] { col, row }));

                } else if (index == 2 && counter == 1) {
                    units[0][(row / boxSize * boxSize + col / boxSize)][((row * boxSize - (row / boxSize * (size)))
                            + (col - (col / boxSize * boxSize)))] = testVal.not();
                    units[1][((row * boxSize - (row / boxSize * (size))) + (col - (col / boxSize * boxSize)))][(row
                            / boxSize * boxSize + col / boxSize)] = testVal.not();
                    units[2][row][col] = testVal.not();
                    board[(row / boxSize * boxSize + col / boxSize)][((row * boxSize - (row / boxSize * (size)))
                            + (col - (col / boxSize * boxSize)))] = testVal;

                    priority.remove(findIndex(priority, new int[] { (row / boxSize * boxSize + col / boxSize),
                            ((row * boxSize - (row / boxSize * (size))) + (col - (col / boxSize * boxSize))) }));
                ...
                index++;
                unitRow = 0;
            } else if ((index == 0 && board[unitRow][unitCol].bitCount() > 1
                    && board[unitRow][unitCol].and(testVal).compareTo(BigInteger.ZERO) != 0) ||
                    (index == 1 && board[unitCol][unitRow].bitCount() > 1
                            && board[unitCol][unitRow].and(testVal).compareTo(BigInteger.ZERO) != 0)
                    ||
                    (index == 2
                            && board[(unitRow / boxSize * boxSize
                                    + unitCol / boxSize)][((unitRow * boxSize - (unitRow / boxSize * (size)))
                                            + (unitCol - (unitCol / boxSize * boxSize)))]
                                    .bitCount() > 1
                            && board[(unitRow / boxSize * boxSize
                                    + unitCol / boxSize)][((unitRow * boxSize - (unitRow / boxSize * (size)))
                                            + (unitCol - (unitCol / boxSize * boxSize)))]
                                    .and(testVal).compareTo(BigInteger.ZERO) != 0)) {
                counter++;
                row = unitRow;
                col = unitCol;
            }
    ...
    // Takes the finsihed units for a sudoku and checks that all the elements in
    // each unit-row is unique, i.e. checks that the sudoku is valid
    private static boolean verification(BigInteger[][][] units) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!(matchRow.equals(BigInteger.ZERO) && matchCol.equals(BigInteger.ZERO)
                    && matchBox.equals(BigInteger.ZERO))) {
                return false;
            }
        }
        return true;
